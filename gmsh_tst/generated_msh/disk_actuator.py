...
def createSceneReal(rootNode, dt):
    ...
    rootNode = Scene(rootNode, gravity=[0.0, -0.0, -10.], dt=dt)
    ...
    sys.exit()
    Bunny.createObject("FixedConstraint", indicies=fixed_const_str)
...
def createScene(rootNode):
    ...
    list_of_vectors = [np.array([float(length_scale), 0, dt])]
    import timeit
    start = timeit.default_timer()
    def animation(target, factor):
        x = 1
        print '\n\n\n\n'
        x = target.disk.dofs.force[mid_ind], mid_ind
        print(x)
        list_of_vectors.append(np.array(x[0]))
    ...
    animate(animation, {"target": rootNode}, duration=1, mode="once", onDone=ExitFunc)
