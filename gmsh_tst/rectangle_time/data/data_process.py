...
def plot_force_over_time(length_scale=None):
    ...
    for i, m in enumerate(arrs):
        ...
        # f = lengths[i] ** -2
        x = np.sum(l, axis=1) * -1
        print("x.shape", x.shape, dt[i])
        forces.extend(x)
        time_ratios.extend(np.ones(x.shape)*time_ratio[i])
        times.extend(np.arange(len(x))*dt[i])
    ...
    plt.show()
    plt.savefig("ForceFixedEndOvertime.png")
...
def dt_vs_compute_time():
    ...
    im = ax.scatter(info[:,2], info[:,1],  c=info[:, 1],norm=cl.LogNorm())
    cb = fig.colorbar(im)
    cb.set_label("Compute Time : Simulation Time Ratio")

    ax.set_ylabel("Computation ratio")
    #alt ax.set_ylabel("number of nodes")

    ax.set_xlabel("dt")


    fig.suptitle(str(info[:,2]))
    #ax.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    plt.savefig("Scale_density_compute_time_alt_zoom.png")
    plt.show()
