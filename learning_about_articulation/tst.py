...
def cube(rootNode, translation=[0.,0.,0.]):
...
class myCube(SofaObject):
    ...
    def __init__(self, parent, translation=[0, 23.64, -45.6], rotation="0 0 0", scale=[1.0,1.0,1.0]):
...
@SofaPrefab
class sm_cube(SofaObject):
    def __init__(self, parentNode, translation=[0., 0., 0.], name="cubehard", mapped_dof=None):
        self.node = parentNode.createChild(name)
        self.mapped_dof = mapped_dof
        cb = Cube(self.node, name="cubehard", uniformScale=5, translation=translation)
        
        if mapped_dof != None:
            cb.createObject('RestShapeSpringsForceField', name="rssff" + str(0),
                                                     points=0,
                                                     external_rest_shape=self.mapped_dof.getLinkPath(), index=1,
                                                     stiffness='1e12', angularStiffness='1e12')

        cb.createObject("LinearSolverConstraintCorrection")
...
def createScene(rootNode):
    ...
    #myCube(scene.Modelling)
    #print(dir(cube.mstate))
    #tripod = Tripod(scene.Modelling, numMotors=1,
    #                mapped_dof=scene.Modelling.articulation.sub_node.arm.DOFs)

    one = sm_cube(scene.Modelling, name="one", translation=[0.0, 10.0, 0.0], mapped_dof=None)
    two = sm_cube(scene.Modelling, name="two", translation=[20.0, 10.0, 0.0], mapped_dof=None)
    scene.Modelling.createObject('BilateralInteractionConstraint', template="Rigid3d",
                            object1=two.cubehard.mstate.getLinkPath(),
                            object2=one.cubehard.mstate.getLinkPath(), first_point="0",
                            second_point="1")
